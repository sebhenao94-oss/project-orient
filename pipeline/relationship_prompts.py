"""Offline prompt-package loading for Project ORIENT relationship mapping.

Mirrors `equipment_prompts` but for relationship inference. Two deliberate
differences from the equipment package:

1. Few-shot examples are text-only (no demonstration images). The Floor-1
   worked example we seed from is a known set of edges, not a matching image,
   and text demonstrations avoid the image few-shot-leakage failure mode found
   in W3 equipment extraction.
2. The single target message carries both the normalised equipment list (text)
   and the target drawing image. The equipment list is injected into the user
   template at the ``<<EQUIPMENT_LIST>>`` sentinel.

This module validates committed prompt artifacts and builds provider-neutral
message plans. It does not encode images, call an LLM, or perform network, S3,
database, or response-parsing work.
"""

from __future__ import annotations

import json
from collections.abc import Mapping
from dataclasses import dataclass, field
from pathlib import Path
from typing import Any, Dict, List, Tuple, Union

from pydantic import ValidationError

if __package__:
    from .models import RelationshipExtractionResponse
else:
    from models import RelationshipExtractionResponse


EQUIPMENT_LIST_PLACEHOLDER = "<<EQUIPMENT_LIST>>"


@dataclass(frozen=True)
class PromptVersionFiles:
    system_prompt_filename: str
    user_template_filename: str
    manifest_filename: str


SUPPORTED_PROMPT_VERSIONS: Dict[str, PromptVersionFiles] = {
    "relationship_mapping_v2": PromptVersionFiles(
        system_prompt_filename="v2_system.md",
        user_template_filename="v2_user_template.md",
        manifest_filename="v2_few_shot_examples.json",
    ),
}


class RelationshipPromptError(ValueError):
    """Base error for relationship prompt-package loading failures."""


class UnsupportedPromptVersionError(RelationshipPromptError):
    """Raised when a requested prompt version is not supported."""


class PromptPackageFileError(RelationshipPromptError):
    """Raised when a prompt package file is missing or unreadable."""


class PromptManifestError(RelationshipPromptError):
    """Raised when a few-shot manifest is malformed or invalid."""


class TargetImageResolutionError(RelationshipPromptError):
    """Raised when a target image path is unsafe or unavailable."""


class UserTemplateError(RelationshipPromptError):
    """Raised when the user template cannot host the equipment list."""


@dataclass(frozen=True)
class RelationshipPromptExample:
    user_text: str
    expected_response: RelationshipExtractionResponse


@dataclass(frozen=True)
class RelationshipPromptPackage:
    prompt_version: str
    system_prompt: str
    user_template: str
    examples: Tuple[RelationshipPromptExample, ...]


@dataclass(frozen=True)
class SystemTextMessage:
    text: str
    role: str = field(default="system", init=False)


@dataclass(frozen=True)
class UserTextMessage:
    text: str
    role: str = field(default="user", init=False)


@dataclass(frozen=True)
class AssistantJsonMessage:
    expected_response: RelationshipExtractionResponse
    json_text: str
    role: str = field(default="assistant", init=False)


@dataclass(frozen=True)
class UserImageTextMessage:
    image_path: Path
    text: str
    role: str = field(default="user", init=False)


RelationshipMessage = Union[
    SystemTextMessage,
    UserTextMessage,
    AssistantJsonMessage,
    UserImageTextMessage,
]


@dataclass(frozen=True)
class RelationshipMessagePlan:
    prompt_version: str
    messages: Tuple[RelationshipMessage, ...]


def load_relationship_prompt_package(
    prompt_version: str,
    prompt_root: Path,
) -> RelationshipPromptPackage:
    """Load and validate one versioned relationship-mapping prompt package."""
    version_files = _version_files(prompt_version)
    prompt_root = Path(prompt_root)

    system_prompt = _read_required_text(
        prompt_root / version_files.system_prompt_filename,
        prompt_version,
        "system prompt",
    )
    user_template = _read_required_text(
        prompt_root / version_files.user_template_filename,
        prompt_version,
        "user template",
    )
    if EQUIPMENT_LIST_PLACEHOLDER not in user_template:
        raise UserTemplateError(
            f"{prompt_version}: user template must contain the "
            f"{EQUIPMENT_LIST_PLACEHOLDER} placeholder"
        )

    manifest = _read_required_manifest(
        prompt_root / version_files.manifest_filename,
        prompt_version,
    )
    examples = _validate_manifest(manifest, prompt_version)

    return RelationshipPromptPackage(
        prompt_version=prompt_version,
        system_prompt=system_prompt,
        user_template=user_template,
        examples=tuple(examples),
    )


def build_relationship_message_plan(
    prompt_package: RelationshipPromptPackage,
    equipment_list_text: str,
    target_image_path: Path,
) -> RelationshipMessagePlan:
    """Build an ordered provider-neutral message plan for one target image."""
    if not isinstance(equipment_list_text, str) or not equipment_list_text.strip():
        raise UserTemplateError("equipment_list_text must be nonblank")

    target_image_path = _resolve_target_image_path(target_image_path)
    rendered_target_text = prompt_package.user_template.replace(
        EQUIPMENT_LIST_PLACEHOLDER, equipment_list_text
    )

    messages: List[RelationshipMessage] = [
        SystemTextMessage(text=prompt_package.system_prompt)
    ]

    for example in prompt_package.examples:
        messages.append(UserTextMessage(text=example.user_text))
        messages.append(
            AssistantJsonMessage(
                expected_response=example.expected_response,
                json_text=_relationship_response_json(example.expected_response),
            )
        )

    messages.append(
        UserImageTextMessage(
            image_path=target_image_path,
            text=rendered_target_text,
        )
    )

    return RelationshipMessagePlan(
        prompt_version=prompt_package.prompt_version,
        messages=tuple(messages),
    )


def _version_files(prompt_version: str) -> PromptVersionFiles:
    try:
        return SUPPORTED_PROMPT_VERSIONS[prompt_version]
    except KeyError as exc:
        raise UnsupportedPromptVersionError(
            f"Unsupported relationship prompt version: {prompt_version}"
        ) from exc


def _read_required_text(path: Path, prompt_version: str, description: str) -> str:
    try:
        if not path.exists():
            raise PromptPackageFileError(
                f"{prompt_version}: missing {description} file: {path.name}"
            )
        if not path.is_file():
            raise PromptPackageFileError(
                f"{prompt_version}: {description} path is not a file: {path.name}"
            )
        text = path.read_text(encoding="utf-8")
    except OSError as exc:
        raise PromptPackageFileError(
            f"{prompt_version}: unable to read {description} file: {path.name}"
        ) from exc

    if not text.strip():
        raise PromptPackageFileError(
            f"{prompt_version}: {description} file must not be blank: {path.name}"
        )
    return text


def _read_required_manifest(path: Path, prompt_version: str) -> Mapping[str, Any]:
    try:
        if not path.exists():
            raise PromptPackageFileError(
                f"{prompt_version}: missing few-shot manifest file: {path.name}"
            )
        if not path.is_file():
            raise PromptPackageFileError(
                f"{prompt_version}: few-shot manifest path is not a file: {path.name}"
            )
        manifest_text = path.read_text(encoding="utf-8")
    except OSError as exc:
        raise PromptPackageFileError(
            f"{prompt_version}: unable to read few-shot manifest file: {path.name}"
        ) from exc

    try:
        manifest = json.loads(manifest_text)
    except json.JSONDecodeError as exc:
        raise PromptManifestError(
            f"{prompt_version}: malformed JSON manifest: {path.name}"
        ) from exc

    if not isinstance(manifest, Mapping):
        raise PromptManifestError(
            f"{prompt_version}: manifest top-level value must be an object: {path.name}"
        )
    return manifest


def _validate_manifest(
    manifest: Mapping[str, Any],
    requested_prompt_version: str,
) -> List[RelationshipPromptExample]:
    manifest_version = manifest.get("prompt_version")
    if not isinstance(manifest_version, str) or not manifest_version.strip():
        raise PromptManifestError(
            f"{requested_prompt_version}: manifest field prompt_version is required"
        )
    if manifest_version != requested_prompt_version:
        raise PromptManifestError(
            f"{requested_prompt_version}: manifest prompt_version {manifest_version} "
            "does not match requested version"
        )

    raw_examples = manifest.get("examples")
    if not isinstance(raw_examples, list):
        raise PromptManifestError(
            f"{requested_prompt_version}: manifest field examples must be a list"
        )
    if not raw_examples:
        raise PromptManifestError(
            f"{requested_prompt_version}: manifest examples list must not be empty"
        )

    examples: List[RelationshipPromptExample] = []
    for index, raw_example in enumerate(raw_examples, start=1):
        if not isinstance(raw_example, Mapping):
            raise PromptManifestError(
                f"{requested_prompt_version}: example {index} must be an object"
            )

        user_text = _required_nonblank_string(
            raw_example,
            "user_text",
            requested_prompt_version,
            index,
        )
        if "expected_response" not in raw_example:
            raise PromptManifestError(
                f"{requested_prompt_version}: example {index} missing required field "
                "expected_response"
            )

        try:
            expected_response = RelationshipExtractionResponse(
                **raw_example["expected_response"]
            )
        except (TypeError, ValidationError) as exc:
            raise PromptManifestError(
                f"{requested_prompt_version}: example {index} has invalid "
                "expected_response"
            ) from exc

        examples.append(
            RelationshipPromptExample(
                user_text=user_text,
                expected_response=expected_response,
            )
        )

    return examples


def _required_nonblank_string(
    raw_example: Mapping[str, Any],
    field_name: str,
    prompt_version: str,
    example_index: int,
) -> str:
    if field_name not in raw_example:
        raise PromptManifestError(
            f"{prompt_version}: example {example_index} missing required field {field_name}"
        )
    value = raw_example[field_name]
    if not isinstance(value, str) or not value.strip():
        raise PromptManifestError(
            f"{prompt_version}: example {example_index} field {field_name} must be "
            "a nonblank string"
        )
    return value


def _resolve_target_image_path(target_image_path: Path) -> Path:
    target_image_path = Path(target_image_path).resolve()
    if not target_image_path.exists():
        raise TargetImageResolutionError(
            f"Target image file does not exist: {target_image_path}"
        )
    if not target_image_path.is_file():
        raise TargetImageResolutionError(
            f"Target image path is not a file: {target_image_path}"
        )
    return target_image_path


def _relationship_response_json(response: RelationshipExtractionResponse) -> str:
    return json.dumps(
        response.model_dump(mode="json"),
        ensure_ascii=False,
        separators=(",", ":"),
    )
