...
EXPECTED_ITEMS = {
    ...
    "VAVRH_2_1.png": [
        ("VAVRH_2_1", "VAVRH_2_1", "VAVRH", 0.99),
    ],
    "fptu_2_01.png": [
        ("FPTU_2_01", "FPTU_2_01", "FPTU", 0.99),
        ("AHU 02 A", "AHU_02A", "AHU", 0.99),
    ],
    "fcu_02_1.png": [
        ("FCU_02_1", "FCU_02_1", "FCU", 0.99),
        ("OAVAV_02_04", "OAVAV_02_04", "OAVAV", 0.99),
    ],
}


class TestEquipmentExtractionPromptV2(unittest.TestCase):
    def load_manifest(self):
        manifest_path = PROMPT_DIR / "v2_few_shot_examples.json"
    ...
    def test_prompt_version_is_v2(self):
        ...
        self.assertEqual(manifest["prompt_version"], "equipment_extraction_v2")
    ...
    def test_filenames_and_order_match_v1(self):
        manifest = self.load_manifest()
        filenames = [example["image_filename"] for example in manifest["examples"]]

        self.assertEqual(filenames, EXPECTED_FILENAMES)

    def test_expected_item_counts_match_ground_truth(self):
        manifest = self.load_manifest()
        counts = [
            len(EquipmentExtractionResponse(**example["expected_response"]).equipment)
    ...
    def test_expected_responses_validate_against_contract(self):
        manifest = self.load_manifest()
        for example in manifest["examples"]:
            EquipmentExtractionResponse(**example["expected_response"])
    ...
    def test_examples_collectively_cover_expected_types(self):
        manifest = self.load_manifest()
        covered_types = set()
        for example in manifest["examples"]:
            response = EquipmentExtractionResponse(**example["expected_response"])
        ...
        self.assertEqual(covered_types, {"AHU", "VAV", "VAVRH", "FPTU", "FCU", "OAVAV"})
    ...
    def test_paths_are_relative_and_safe(self):
        manifest = self.load_manifest()
        for example in manifest["examples"]:
    ...
    def test_prompt_templates_are_non_empty(self):
        self.assertTrue((PROMPT_DIR / "v2_system.md").read_text(encoding="utf-8").strip())
        self.assertTrue(
            (PROMPT_DIR / "v2_user_template.md").read_text(encoding="utf-8").strip()
        )

    def test_system_prompt_includes_all_visible_equipment_behavior(self):
        prompt = (PROMPT_DIR / "v2_system.md").read_text(encoding="utf-8").lower()
    ...
    def test_user_template_excludes_non_equipment_from_candidate_list_only(self):
        prompt = (PROMPT_DIR / "v2_user_template.md").read_text(encoding="utf-8").lower()
